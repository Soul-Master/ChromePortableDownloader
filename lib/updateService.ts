...
export type RequestConfigType = typeof requestConfig;
export type ConfigNameType = keyof RequestConfigType;

export async function checkUpdate(configName: ConfigNameType, targetVersion?: string) {
...
function parseXmlResponse(xml: string): UpdateInfo | null {
    const parser = new DOMParser();
    const doc = parser.parseFromString(xml, "application/xml");
    const updateResult = doc.querySelector('updatecheck');

    if (!updateResult || updateResult.getAttribute('status') !== 'ok') return null;

    const manifest = updateResult.querySelector('manifest');
    if (!manifest) return null;

    const version = <string>manifest.getAttribute('version');
    ...
    if (!actionNode || !packageNode) return null;

    const fileName = actionNode.getAttribute('run');
    const fileSize = parseInt(packageNode.getAttribute('size') || '', 10);
    const hash = <string>packageNode.getAttribute('hash');
    const hashSha256 = <string>packageNode.getAttribute('hash_sha256');
    const urls: string[] = [];
